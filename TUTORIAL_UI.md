...
- **快捷键**：
  ...
  - `zM`：**一键折叠全文**所有函数与类
  - `zR`：**一键展开全文**所有折叠
  - 󰆤 **鼠标点击折叠三角 (IDE 级交互)**：行号旁侧自动显示可折叠三角（`` 展开 / `` 折叠），直接用鼠标左键点击三角即可折叠/展开代码块！
- **演练**：在 `ui_playground.py` 场景 03（第 75 行），观察左侧行号旁的 `` 三角图标，用鼠标直接点击它测试折叠；或把光标移到 `def long_calculation_algorithm` 行按 `zc`，然后按 `zp` 查看弹窗！
